	...
	<body background="i14.jpg">
		...
		<form method="post">
			<h1 style="margin-top:10%;  margin-left:25%;
				font-size:50px;">Check bills cancelled</h1>
		...
		<?php
				if(isset($_POST['submit']))
				{
					...
					$sql="SELECT * from cbill1 WHERE date >= '$newdate' and date <= '$newdate2' and $newdate2>$newdate ";
					...
					if($count > 0)
					{
						...
						while($rows=mysqli_fetch_assoc($result))
						{
								...
								$d=$rows['date'];
								?>
										<form method="post" target="leftframe" action ="m3_1.php">
										<tr>
					...
					else
					{
						?>
								...
								<script>						
								alert("No bills cancelled in this period");
								</script>
